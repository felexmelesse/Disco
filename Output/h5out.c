...
//Building Tracer Buffer
   ...
   int Ntr_tot  = theDomain->theTracers->size;
   int myNtr    = Ntr_tot;
   ...
   for( nrk=0 ; nrk < size ; ++nrk ){
      if( rank==nrk ){      
         ...
         glo_size2[0] = Ntr_tot;
         glo_size2[1] = Nints;
         writePatch( filename, "Data", "Tracer_id", trInts, H5T_NATIVE_INT, 2, start2, loc_size2, glo_size2 );
